...
Twelve files carry one today — `modules/nixpkgs.nix`, `modules/prefer-path.nix`,
`modules/clipboard.nix`, `modules/auto-complete.nix` and `modules/session.nix`,
`modules/formatter.nix` (two),
...
`modules/languages/csharp.nix` (eight) and `modules/neovide.nix`. The thirteenth is
